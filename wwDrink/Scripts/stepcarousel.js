...
Stepcarousel = function () {
    ...
    self.alignpanels = function($, config) {
        ...
        config.$panels.each(function(index) { //loop through panels
            ...
            paneloffset += self.getCSSValue($currentpanel.css('marginRight')) + parseInt($currentpanel.get(0).offsetWidth || $currentpanel.css('width')); //calculate next panel offset
    ...
    self.stepTo = function(galleryid, pindex) { /*User entered pindex starts at 1 for intuitiveness. Internally pindex still starts at 0 */
        ...
        self.stopautostep(config);
        pindex = Math.min(pindex - 1, config.paneloffsets.length - 1);
    ...
    self.stepBy = function(galleryid, steps, isauto) {
        ...
        if (config.panelbehavior.wraparound == false) { //if carousel viewer should stop at first or last panel (instead of wrap back or forth)
            ...
            if (config.defaultbuttons.enable == true) { //if default nav buttons are enabled, fade them in and out depending on if at start or end of carousel
                self.fadebuttons(config, pindex);
            }
    ...
    self.createpaginate = function($, config) {
        if (config.$paginatediv.length == 1) {
            ...
            var controlsrc = [];
            var imgarray = [];
            var moveby = $templateimg.attr("data-moveby") || 1;
    ...
    self.init = function($, config) {
        ...
        self.configholder[config.galleryid] = config; //store config parameter as a variable
        if (config.contenttype[0] == "ajax" && typeof config.contenttype[1] != "undefined") //fetch ajax content?
            self.getremotepanels($, config);
        else
    ...
    self.unload = function (config) { //clean up on page unload
        self.resetsettings($, config);
        if (config.panelbehavior.persist)
            self.setCookie(config.galleryid + "persist", config.currentpanel);
